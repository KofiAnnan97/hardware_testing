...
/*
Tested on MEGA 2560 using the WT901B sensor. Based on the sample code from 
WitMotion Arduino SDK found here: 
...
float *angularVelocity = fGyro;
float *linearAcceleration = fAcc;
...
void loop() {
    ...
    if(s_cDataUpdate)
    {
      ...
      if(s_cDataUpdate & ANGLE_UPDATE)
      {
        ...
        float roll = ToRadian(fAngle[0]);
        float pitch = ToRadian(fAngle[1]);
        float yaw =  ToRadian(fAngle[2]);
